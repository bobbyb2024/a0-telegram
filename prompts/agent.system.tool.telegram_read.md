## telegram_read
Read messages from Telegram chats, list recent chats, list forum topics, or get chat info.
...
**Arguments:**
- **action** (string): `messages`, `chats`, `chat_info`, or `topics`
- **chat_id** (string): Chat/group/channel ID (required for `messages`, `chat_info`, `topics`)
- **thread_id** (string): Forum topic thread ID to filter messages by topic (optional, for `messages`)
...
~~~
~~~json
{"action": "messages", "chat_id": "-1001234567890", "thread_id": "42", "limit": "30"}
~~~
~~~json
...
- Use `topics` to list forum topics in a supergroup (requires bridge to have received topic events)
- Use `thread_id` with `messages` to read messages from a specific forum topic
